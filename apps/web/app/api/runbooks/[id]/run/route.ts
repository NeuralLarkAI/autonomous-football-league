...
export async function POST(
  ...
  try {
    ...
    if (runbook.actionType === "RUN_AGENT") {
      ...
      const season = await prisma.season.findFirst({ where: { leagueId: activeLeague.id, seasonNumber: 1 } });
      if (!season) throw new Error("Season 1 not found");
      const games = await prisma.game.findMany({
        where: { leagueId: activeLeague.id, seasonId: season.id, week: weekNumber },
        include: { homeTeam: true, awayTeam: true },
        orderBy: { kickoffAt: "asc" },
      });
      ...
      if (!season) throw new Error("Season 1 not found");
      const games = await prisma.game.findMany({
        where: { leagueId: activeLeague.id, seasonId: season.id, week: weekNumber, status: "FINAL" },
        include: { homeTeam: true, awayTeam: true },
        orderBy: { kickoffAt: "asc" },
      });
      const post = await ensureSocialPost({
      ...
      const remainingWeeks = await prisma.game.findMany({
        where: { leagueId: activeLeague.id, seasonId: season.id, status: { not: "FINAL" } },
        select: { week: true },
        distinct: ["week"],
      });
