...
import { runAgent, runKickoff, runCombine, stepGame, generateAgentNarrative, hasClaudeAgentBrain } from "@afl/agents";
...
export async function executeRunbookAction(runbook: RunbookLike, runbookRunId: string): Promise<string> {
  ...
  if (runbook.actionType === "RUN_AGENT") {
    ...
    const [openTasks, pendingApprovals, openIncidents, scheduledRunbooks] = await Promise.all([
      prisma.task.count({
        where: { leagueId: runbook.leagueId, status: { in: ["BACKLOG", "IN_PROGRESS", "REVIEW", "BLOCKED"] } },
      }),
      prisma.approval.count({ where: { leagueId: runbook.leagueId, status: "PENDING" } }),
    ...
    const message = await prisma.message.create({
      data: {
        ...
        body:
          aiReportBody ??
          `Generated by runbook ${runbook.name} at ${new Date().toISOString()}\n- Open tasks: ${openTasks}\n- Pending approvals: ${pendingApprovals}\n- Open incidents: ${openIncidents}`,
    ...
    if (aiReportBody) {
      await prisma.eventLog.create({
        data: {
          ...
          summary: `${agent.name} generated AI-assisted weekly report content.`,
          entityType: "AGENT",
          entityId: agent.id,
      ...
      await prisma.eventLog.create({
        data: {
          ...
          summary: `${agent.name} report fell back to deterministic template.`,
          entityType: "AGENT",
          entityId: agent.id,
          meta: JSON.stringify({ provider: "anthropic", mode: "runbook_report", runbookId: runbook.id }),
        },
